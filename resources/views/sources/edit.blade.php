...
@section('page-title', 'Edit Source')

@section('page-description', 'Update source information and settings')

@section('header-actions')
    <div class="flex space-x-3">
        <a href="{{ route('sources.show', $source->id) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
            <i class="fas fa-eye mr-2"></i>View Source
        </a>
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow">
        <form method="POST" action="{{ route('sources.update', $source->id) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" required
                            ...
                            <option value="social_media" {{ old('type', $source->type) === 'social_media' ? 'selected' : '' }}>Social Media</option>
                            <option value="email" {{ old('type', $source->type) === 'email' ? 'selected' : '' }}>Email Marketing</option>
                            <option value="referral" {{ old('type', $source->type) === 'referral' ? 'selected' : '' }}>Referral</option>
                            <option value="advertising" {{ old('type', $source->type) === 'advertising' ? 'selected' : '' }}>Paid Advertising</option>
                            <option value="direct" {{ old('type', $source->type) === 'direct' ? 'selected' : '' }}>Direct</option>
                    ...
                    <div>
                        ...
                        <input type="text" id="tracking_code" name="tracking_code" value="{{ old('tracking_code', $source->tracking_code) }}"
                ...
                <div class="mt-6">
                    ...
                    <textarea id="description" name="description" rows="3"
                              ...
                              placeholder="Describe this lead source...">{{ old('description', $source->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" id="cost_per_lead" name="cost_per_lead" value="{{ old('cost_per_lead', $source->cost_per_lead) }}" 
                               step="0.01" min="0"
                    ...
                    <div>
                        ...
                        <input type="number" id="monthly_budget" name="monthly_budget" value="{{ old('monthly_budget', $source->monthly_budget) }}" 
                               step="0.01" min="0"
            ...
            <div class="mb-8 bg-gray-50 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span class="text-gray-600">{{ date('M j, Y g:i A', strtotime($source->created_at)) }}</span>
                    </div>
                    <div>
                        <span class="font-medium text-gray-700">Last Updated:</span>
                        <span class="text-gray-600">{{ date('M j, Y g:i A', strtotime($source->updated_at)) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                <a href="{{ route('sources.show', $source->id) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-6 py-2 rounded-lg font-medium transition-colors">
                    Cancel
                </a>
    ...
    <div class="mt-6 bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        <p class="text-sm text-red-800 mb-3">Deleting this source will permanently remove it from the system. This action cannot be undone.</p>
        <form method="POST" action="{{ route('sources.destroy', $source->id) }}" class="inline" 
            ...
            <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
                <i class="fas fa-trash mr-2"></i>Delete Source
            </button>
        </form>
    </div>
</div>
